...
/* today timeline (schedule view) — title on its own row, track beneath */
.timeline{margin:8px 0 4px}
.tl-axis{position:relative;height:15px;margin-bottom:8px}
...
.tl-head{margin-bottom:4px;line-height:1.3}
.tl-nm{font-weight:700;font-size:15px}
.tl-nm .cbadge{margin-left:6px}
.tl-sub{color:var(--muted);font-size:12.5px;margin-left:2px}
.tl-track{position:relative;height:26px;border-radius:6px;background:var(--card);border:1px solid var(--line)}
.tl-bar{position:absolute;top:-1px;height:26px;box-sizing:border-box;background:var(--accent);color:#04121f;border-radius:5px;font-size:12.5px;font-weight:700;line-height:24px;text-align:center;min-width:40px;white-space:nowrap;box-shadow:0 1px 2px rgba(0,0,0,.3)}
...
@media(max-width:760px){.tl-nm{font-size:14px}.tl-sub{font-size:12px}.tl-bar{font-size:12px;min-width:34px}}
...
function timelineHtml(open,key){
  ...
  // keep the caller's order when sorting by distance ("närmast mig"); otherwise
  // order by earliest opening (then earliest closing), unparsed shops last
  if(!(sortSel.value==='dist' && refPt)){
    rows.sort((a,b)=>{
  ...
  rows.forEach(r=>{
    ...
    out+='<div class="tl-item"><div class="tl-head">'+
