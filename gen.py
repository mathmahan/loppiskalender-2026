...
/* today timeline (schedule / gantt view) */
:root{--tlname:210px}
.timeline{margin:8px 0 4px;overflow-x:auto;-webkit-overflow-scrolling:touch}
.tl-inner{min-width:560px}
.tl-axis{display:flex;align-items:flex-end;height:16px;margin-bottom:6px}
.tl-axis .tl-spacer{flex:0 0 var(--tlname)}
.tl-scale{position:relative;flex:1;height:100%}
.tl-tick{position:absolute;top:0;transform:translateX(-50%);font-size:11px;color:var(--muted);white-space:nowrap}
.tl-row{display:flex;align-items:center;min-height:32px;border-radius:6px;padding:1px 0}
.tl-row:hover{background:rgba(79,155,232,.07)}
.tl-name{flex:0 0 var(--tlname);width:var(--tlname);padding-right:12px;overflow:hidden}
.tl-nm{font-weight:600;font-size:13.5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block}
.tl-nm .cbadge{margin-left:5px}
.tl-sub{color:var(--muted);font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block}
.tl-track{position:relative;flex:1;height:26px;border-radius:6px;background:var(--card);border:1px solid var(--line)}
.tl-bar{position:absolute;top:-1px;height:26px;box-sizing:border-box;background:var(--accent);color:#04121f;border-radius:5px;font-size:12px;font-weight:700;line-height:24px;text-align:center;min-width:38px;white-space:nowrap;box-shadow:0 1px 2px rgba(0,0,0,.3)}
.tl-bar.season{background:#2f6aa3;color:#eaf3fd;border:1px dashed #7fb4e6}
.tl-bar.noparse{background:transparent;color:var(--muted);border:1px dashed var(--line);font-weight:600;box-shadow:none}
@media(max-width:760px){:root{--tlname:104px}.tl-inner{min-width:0}.tl-name{padding-right:8px}.tl-nm{font-size:12.5px}.tl-sub{font-size:10.5px}.tl-tick{font-size:10px}.tl-bar{font-size:11px;min-width:30px;letter-spacing:-.2px}}
@media(max-width:380px){:root{--tlname:88px}}
...
// parse an hours string like "10–16", "9:30–13:30" into {s,e} minutes-from-midnight, or null
function parseHours(str){
  if(!str) return null;
  const m=String(str).replace(/[.]/g,':').match(/([0-9]{1,2})(?::([0-9]{2}))?[ ]*[–-][ ]*([0-9]{1,2})(?::([0-9]{2}))?/);
  if(!m) return null;
  const s=(+m[1])*60+(+(m[2]||0)), e=(+m[3])*60+(+(m[4]||0));
  if(!(e>s)) return null;
  return {s,e};
}
...
// build a schedule/gantt timeline of shops open today so opening times are visible at a glance
function timelineHtml(open,key){
  const rows=open.map(m=>({m,ph:parseHours(m.days[key])}));
  let lo=Infinity,hi=-Infinity;
  rows.forEach(r=>{ if(r.ph){ lo=Math.min(lo,r.ph.s); hi=Math.max(hi,r.ph.e); } });
  let minH,maxH;
  if(lo===Infinity){ minH=8; maxH=20; } else { minH=Math.floor(lo/60); maxH=Math.ceil(hi/60); }
  if(maxH-minH<2) maxH=minH+2;
  const span=maxH-minH, hourPct=100/span;
  // sort: earliest opening first, then earliest closing; unparsed shops last
  rows.sort((a,b)=>{
    if(!a.ph&&!b.ph) return 0; if(!a.ph) return 1; if(!b.ph) return -1;
    return a.ph.s-b.ph.s || a.ph.e-b.ph.e;
  });
  const step=span<=8?1:(span<=14?2:3);
  let ticks='';
  for(let h=minH;h<=maxH;h+=step){
    const pct=(h-minH)/span*100;
    const tf=pct<=0?'none':(pct>=100?'translateX(-100%)':'translateX(-50%)');
    ticks+='<div class="tl-tick" style="left:'+pct+'%;transform:'+tf+'">'+h+'</div>';
  }
  const trackStyle='background-image:repeating-linear-gradient(90deg,var(--line) 0,var(--line) 1px,transparent 1px,transparent '+hourPct+'%)';
  let out='<div class="timeline"><div class="tl-inner">'+
          '<div class="tl-axis"><div class="tl-spacer"></div><div class="tl-scale">'+ticks+'</div></div>';
  rows.forEach(r=>{
    const m=r.m, badge=m.season?'<span class="cbadge">säsong</span>':'';
    const title=[m.mun,m.region,m.typ||'',m.addr||''].filter(Boolean).join(' · ');
    let bar;
    if(r.ph){
      const left=(r.ph.s-minH*60)/(span*60)*100, width=(r.ph.e-r.ph.s)/(span*60)*100;
      bar='<div class="tl-bar'+(m.season?' season':'')+'" style="left:'+left+'%;width:'+width+'%">'+esc(m.days[key])+'</div>';
    } else {
      bar='<div class="tl-bar noparse" style="left:0;width:100%">'+esc(m.days[key]||'–')+'</div>';
    }
    out+='<div class="tl-row"><div class="tl-name" title="'+esc(title)+'">'+
         '<span class="tl-nm">'+esc(m.name)+badge+'</span>'+
         '<span class="tl-sub">'+esc(m.mun)+' · '+esc(m.typ||'')+distChip(m)+'</span></div>'+
         '<div class="tl-track" style="'+trackStyle+'">'+bar+'</div></div>';
  });
  out+='</div></div>';
  return out;
}
